...
# .repartition(1)\
  # for index,res in enumerate(res_table):
  #   res.map(lambda x: (x[0], x[1] , x[2],x[3],x[4])).toDF(["year", "date" , "median","low","high"])\
  #                                                   .write.format("csv")\
  #                                                   .option("header", "true")\
  #                                                   .save(path_name[index].replace('test',sys.argv[1]) if len(sys.argv)>1 else path_name[index] )
  #                                                   # .save(path_name[index])